...
mkdir -p ${INSTALL_DIR}
\cp -r web_api/ ${INSTALL_DIR}
cd ${INSTALL_DIR}/web_api/bin
...
sh -c "sed -i 's@JAVA_OPTS=.*@JAVA_OPTS=$JAVA_OPTS@' bat-web-api.sh"

\cp -f $CURRENT_DIR/monitor-web_api.service /usr/lib/systemd/system
systemctl enable monitor-web_api.service
