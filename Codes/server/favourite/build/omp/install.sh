...
mkdir -p ${INSTALL_DIR}
\cp -r favourite/ ${INSTALL_DIR}
cd ${INSTALL_DIR}/favourite/bin
...
sh -c "sed -i 's@JAVA_OPTS=.*@JAVA_OPTS=$JAVA_OPTS@' bat-favourite.sh"

\cp -f $CURRENT_DIR/monitor-favourite.service /usr/lib/systemd/system
systemctl enable monitor-favourite.service
