...
sh -c "sed -i 's@JAVA_OPTS=.*@JAVA_OPTS=$JAVA_OPTS@' bat-event.sh"

\cp -f $CURRENT_DIR/monitor-event.service /usr/lib/systemd/system
systemctl enable monitor-event.service
