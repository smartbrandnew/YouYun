...
mkdir -p ${INSTALL_DIR}
\cp -r gateway/ ${INSTALL_DIR}
cd ${INSTALL_DIR}/gateway/bin
...
sh -c "sed -i 's@JAVA_OPTS=.*@JAVA_OPTS=$JAVA_OPTS@' bat-gateway.sh"

\cp -f $CURRENT_DIR/monitor-gateway.service /usr/lib/systemd/system
systemctl enable monitor-gateway.service
