...
mkdir -p ${INSTALL_DIR}
\cp -r monitor/ ${INSTALL_DIR}
cd ${INSTALL_DIR}/monitor/bin
...
sh -c "sed -i 's@JAVA_OPTS=.*@JAVA_OPTS=$JAVA_OPTS@' bat-monitor.sh"

\cp -f $CURRENT_DIR/monitor-monitor.service /usr/lib/systemd/system
systemctl enable monitor-monitor.service
