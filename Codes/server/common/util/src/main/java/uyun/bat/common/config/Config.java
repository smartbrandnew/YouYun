...
public class Config extends AbstractDynamicObject {
	...
	/**
	 * 初始化系统，加载配置到系统中，方便spring的各类属性注入
	 * 默认加载monitor.properties和common.properties
	 */
	private void loadLocalProperties() {
		if (!loadLocalProperties("monitor.properties") || !loadLocalProperties("common.properties"))
			logger.warn("Cannot load default local properties files");
		loadLocalProperties("config-developer.properties");
	}
	...
	private static File searchFile(String filename) {
		String[] searchPaths = new String[] {
				...
				"/../../build/src/main/resources/conf/",
				"/../../server/build/src/main/resources/conf/",
				"/server/build/src/main/resources/conf/"};
	...
	public Object get(String key) {
		String value = searchProperty(key);
		if (value != null) {
			while (true) {
