...
mkdir -p ${INSTALL_DIR}
\cp -r report/ ${INSTALL_DIR}
cd ${INSTALL_DIR}/report/bin
...
sh -c "sed -i 's@JAVA_OPTS=.*@JAVA_OPTS=$JAVA_OPTS@' bat-report.sh"

\cp -f $CURRENT_DIR/monitor-report.service /usr/lib/systemd/system
systemctl enable monitor-report.service
