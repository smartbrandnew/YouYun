...
from circle.errors import MessageError, ArgumentError
...
class Stats(Command):
    """\
       ...
       ::
            ...
            $ circlectl stats [--extended] [<watchername>] [<processid>]

        """

    name = "stats"
    options = [('', 'extended', False, "Include info from extended_stats hook")]
    ...
    def message(self, *args, **opts):
        ...
        if len(args) == 2:
            return self.make_message(
                name=args[0], process=int(args[1]), extended=extended)
        elif len(args) == 1:
