...
class ArbiterHandler(object):
    ...
    def __call__(self,
            ...
            - **stdout_stream**: a mapping containing the options for
              ...
              the stdout stream. Default to None. When provided, may contain:
                ...
                - **class**: the fully qualified name of the class to use for
                             streaming. Defaults to circle.stream.FileStream
        ...
        - **controller** -- the tcp entry point
          (default: '127.0.0.1:5555')
        - **stats_endpoint** -- the stats endpoint. If not provided,
          the *circled-stats* process will not be launched. (default: None)
        - **statsd_close_outputs** -- if True sends the circled-stats
        ...
        - **debug** -- If True the arbiter is launched in debug mode
          (default: False)
        - **proc_name** -- the arbiter process name (default: circled)
        ...
        Arbiter = self._get_arbiter_class(background=background)
    ...
    def _get_arbiter_class(self, background):
        if background:
            pass
        else:
