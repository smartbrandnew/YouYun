...
from circle.util import cast_string
...
class _FileStreamBase(object):
    ...
    def write_data(self, data):
        ...
        file_data = cast_string(data['data'])
...
class FileStream(_FileStreamBase):
    ...
    def __init__(self,
        ...
        Here is an example: ::
          ...
          stdout_stream.filename = /var/log/circle/out.log
...
class WatchedFileStream(_FileStreamBase):
    ...
    def __init__(self, filename=None, time_format=None, **kwargs):
        ...
        self.st_dev, self.st_ino = -1, -1
        self._statfile()

    def _statfile(self):
        stb = os.fstat(self._file.fileno())
        self.st_dev, self.st_ino = stb[ST_DEV], stb[ST_INO]
    ...
    def __call__(self, data):
        ...
        st_dev, st_ino = self._statfilename()
        if st_dev != self.st_dev or st_ino != self.st_ino:
            self._file.flush()
            self._file.close()
            self._file = self._open()
            self._statfile()
...
class TimedRotatingFileStream(FileStream):
    ...
    def __init__(self,
        ...
        '''
        super(TimedRotatingFileStream, self).__init__(
            filename=filename,
