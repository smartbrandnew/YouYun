...
from circle.client import make_json
from circle.util import IS_WINDOWS


class SysHandler(object):

    _SIGNALS_NAMES = ("ILL ABRT BREAK INT TERM"
                      if IS_WINDOWS else "HUP QUIT INT TERM WINCH")
    ...
    def reload(self):
        # We need to transfer the control to the loop's thread
        self.controller.loop.add_callback_from_signal(
            self.controller.dispatch, (None, make_json(
                "reload", graceful=True)))
