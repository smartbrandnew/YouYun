...
if [ -n "$DIR" ]; then
    ...
    dir=/out
    printf "install directory not available in DIR environment variable. use /opt"
fi
...
function sed_file_install_dir() {
    $sudo_cmd sh -c "sed -i 's%/opt%$dir%g' deploy.sh"
    $sudo_cmd sh -c "sed -i 's%/opt%$dir%g' etc/monitor-agent/supervisor.conf"
    $sudo_cmd sh -c "sed -i 's%/opt%$dir%g' postinst preinst datamonitor-agent"
    cd opt/datadog-agent/bin/
    $sudo_cmd sh -c "sed -i 's%/opt%$dir%g' dogwrap libsmi2pysnmp supervisord dog echo_supervisord_conf pidproxy snakebite supervisorctl wsdump.py"
    cd ../../../opt/datadog-agent/agent/
    $sudo_cmd sh -c "sed -i 's%/opt%$dir%g' agent.py monitoragent.py monitorstatsd.py updater_process.py"
    cd ../../../
}

function sed_conf_install_dir() {
...
if [ $OS = "RedHat" ]; then
   ...
   # 下载安装包
    $dl_cmd $repertory_url/downloads/agent/$OS/Monitor-Agent-deb-$ARCHI.tar.gz > monitor.tar.gz
