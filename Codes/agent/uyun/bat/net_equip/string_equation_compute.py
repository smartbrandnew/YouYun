...
def l1_analysis(s):  # 定义括号式分解函数
    s = s.replace(' ', '')
    if s.find('(') == -1:
        return nomal_numerator(s)
    else:
        from_str = l1_pattern.search(s).group()
        s = s.replace(from_str, nomal_numerator(from_str[1:-1]))
        return l1_analysis(s)
    '''
