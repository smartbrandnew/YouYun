...
class monitorstreams(object):
    ...
    def _instantiate_monitorstreams(cls, logger, config, monitorstreams_config):
        """
        Expecting monitorstreams config value to look like:
           <monitorstream value>, <monitor stream value>, ...
        Where <monitorstream value> looks like:
           <log path>
        or
           <log path>:<module>:<parser function>
        """
        monitorstreams = []
        # Create a monitorstream object for each <monitorstream value>
    ...
    def check(self, agentConfig, move_end=True):
        ...
        for monitorstream in self.monitorstreams:
            try:
                ...
                # result may contain {"monitorstream": [new]}.
                # If output contains {"monitorstream": [old]}, that old value will get concatenated with the new value
...
class monitorstream(object):
    ...
    def _aggregate(self, values):
        """ Aggregate values down to the second and store as:
            {
                "monitorstream": [(metric, timestamp, value, {key: val})]
            }
