...
namespace :ci do
  namespace :postgres do |flavor|
    ...
    task install: ['ci:common:install'] do
      ...
      unless Dir.exist? File.expand_path(pg_rootdir)
        sh %(curl -s -L\
             ...
             https://s3.amazonaws.com/monitor-agent-tarball-mirror/#{pg_version}.tar.gz)
    ...
    task before_script: ['ci:common:before_script'] do
      ...
      sh %(#{pg_rootdir}/bin/psql\
           -p 15432 -U datamonitor\
           datamonitor_test < $TRAVIS_BUILD_DIR/ci/resources/postgres/datamonitor_test.sql)
      sh %(#{pg_rootdir}/bin/psql\
           -p 15432 -U datamonitor\
           monitors < $TRAVIS_BUILD_DIR/ci/resources/postgres/monitors.sql)
    end
