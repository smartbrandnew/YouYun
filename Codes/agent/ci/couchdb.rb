    ...
    task install: ['ci:common:install'] do
      if !Dir.exist? File.expand_path(couchdb_rootdir)
        ...
        sh %(curl -s -L\
             ...
             https://s3.amazonaws.com/monitor-agent-tarball-mirror/otp_src_17.4.tar.gz)
        sh %(curl -s -L\
             -o $VOLATILE_DIR/couchdb-#{couchdb_version}.tar.gz\
             https://s3.amazonaws.com/monitor-agent-tarball-mirror/apache-couchdb-#{couchdb_version}.tar.gz)
        sh %(curl -s -L\
             -o $VOLATILE_DIR/js185.tar.gz\
             https://s3.amazonaws.com/monitor-agent-tarball-mirror/js185-1.0.0.tar.gz)
        sh %(curl -s -L\
             -o $VOLATILE_DIR/icu.tar.gz\
             https://s3.amazonaws.com/monitor-agent-tarball-mirror/icu4c-54_1-src.tgz)
