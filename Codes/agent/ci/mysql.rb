...
namespace :ci do
  namespace :mysql do |flavor|
    ...
    task before_script: ['ci:common:before_script'] do
      sh %(mysql -e "create user 'monitor'@'localhost' identified by 'monitor'" -uroot)
      sh %(mysql -e "GRANT PROCESS, REPLICATION CLIENT ON *.* TO 'monitor'@'localhost' WITH MAX_USER_CONNECTIONS 5;" -uroot)
      sh %(mysql -e "CREATE DATABASE testdb;" -uroot)
      sh %(mysql -e "CREATE TABLE testdb.users (name VARCHAR(20), age INT);" -uroot)
      sh %(mysql -e "GRANT SELECT ON testdb.users TO 'monitor'@'localhost';" -uroot)
      ...
      sh %(mysql -e "GRANT SELECT ON performance_schema.* TO 'monitor'@'localhost';" -uroot)
    ...
    task cleanup: ['ci:common:cleanup'] do
      sh %(mysql -e "DROP USER 'monitor'@'localhost';" -uroot)
