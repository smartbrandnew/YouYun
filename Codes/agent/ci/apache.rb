    ...
    task install: ['ci:common:install'] do
      unless Dir.exist? File.expand_path(apache_rootdir)
        ...
        sh %(curl -s -L\
             ...
             https://s3.amazonaws.com/monitor-agent-tarball-mirror/httpd-#{apache_version}.tar.bz2)
        sh %(curl -s -L\
             -o $VOLATILE_DIR/apr.tar.bz2\
             https://s3.amazonaws.com/monitor-agent-tarball-mirror/apr-1.5.1.tar.bz2)
        sh %(curl -s -L\
             -o $VOLATILE_DIR/apr-util.tar.bz2\
             https://s3.amazonaws.com/monitor-agent-tarball-mirror/apr-util-1.5.4.tar.bz2)
