...
class TestCheckDockerDaemon(AgentCheckTest):
    ...
    def tearDown(self):
        for c in self.containers:
            log.info("Stopping net_equip_details: {0}".format(c))
    ...
    def test_container_size(self):
        expected_metrics = [
            ...
            ("docker.net_equip_details.size_rootfs", ['container_name:test-new-nginx', 'docker_image:nginx', 'image_name:nginx']),
            ("docker.net_equip_details.size_rootfs", ['container_name:test-new-redis-latest', 'docker_image:redis:latest', 'image_name:redis', 'image_tag:latest']),
            ("docker.net_equip_details.size_rw", ['container_name:test-new-nginx', 'docker_image:nginx', 'image_name:nginx']),
