...
class TestPostgres(AgentCheckTest):
    ...
    def test_checks(self):
        host = 'localhost'
        port = 15432
        dbname = 'datamonitor_test'

        instances = [
            {
                'host': host,
                'port': port,
                'username': 'datamonitor',
                'password': 'datamonitor',
                'dbname': dbname,
                'relations': ['persons'],
                'custom_metrics': [{
                    ...
                    'query': "SELECT datname, %s FROM pg_stat_database WHERE datname = 'datamonitor_test' LIMIT(1)",
                    'relation': False,
                }]
            },
            {
        ...
        for mname in COMMON_METRICS:
            for db in ('datamonitor_test', 'monitors'):
        ...
        for inst in instances:
            for rel in inst.get('relations', []):
                ...
                for mname in RELATION_METRICS:
                    ...
                    # in the monitors DB, so the other index metrics shouldn't be
                    # here.
        ...
        expected_tags = ['db:monitors', 'table:breed', 'index:breed_names', 'schema:public']
        ...
        # Our custom metric
        self.assertMetric('custom.numbackends', value=1, tags=['customdb:datamonitor_test'])

        # Test service checks
        self.assertServiceCheck('postgres.can_connect',
            count=1, status=AgentCheck.OK,
            tags=['host:localhost', 'port:15432', 'db:datamonitor_test']
        )
        self.assertServiceCheck('postgres.can_connect',
            count=1, status=AgentCheck.OK,
            tags=['host:localhost', 'port:15432', 'db:monitors']
        )
