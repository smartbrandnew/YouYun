...
class TestPostfix(unittest.TestCase):
    #
    # you can execute this monitor-agent unit test via python's nose tool
    #
    # example:
    #
    #     nosetests --nocapture --tests=monitor-agent/tests/test_postfix.py
    #
    def setUp(self):
    ...
    def test_checks(self):
        ...
        # output postfix.py monitor-agent plugin counts... per queue
        print
        for tuple in out_count:
            queue = tuple[3]['tags'][0].split(':')[1]
            self.assertEquals(int(tuple[2]), int(self.in_count[queue][0]))
            print 'Test messages counted by monitor-agent for', queue, '= ', tuple[2]

        #
        # uncomment this to see the raw monitor-agent metric output
        #
        # print out_count
