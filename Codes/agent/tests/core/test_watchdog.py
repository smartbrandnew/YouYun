...
class TestWatchmonitor(unittest.TestCase):
    """Test watchmonitor in various conditions
    """

    JITTER_FACTOR = 2

    def test_watchmonitor(self):
        """Verify that watchmonitor kills ourselves even when spinning
        Verify that watchmonitor kills ourselves when hanging
        """
        start = time.time()
        try:
        ...
        try:
            ...
            self.fail("Watchmonitor killed normal process after %s seconds" % int(time.time() - start))
        ...
        # Slow tornado, killed by the Watchmonitor
        start = time.time()
...
class MemoryHogTxManager(object):
    def __init__(self, watchmonitor):
        self._watchmonitor = watchmonitor
...
class PseudoAgent(object):
    ...
    def slow_tornado(self):
        ...
        a._watchmonitor = Watchmonitor(4)
