...
def get_mocked_config():
    return {
        'api_key': 'APIKEY',
        'm_url': 'https://app.datamonitorhq.com',
    }
...
password_tests = {
    'uri_password' : '      -   server: mongodb://datamonitor:V3pZC7ghx1ne82XkyqLnOW36@localhost:27017/admin',
    'uri_password_2' : '      -   server: mongodb://datamonitor:V3!pZC7ghx1ne8#-2XkyqLnOW36!?@localhost:27017/admin',
    'uri_password_expected' : '      -   server: mongodb://datamonitor:********@localhost:27017/admin',
}
...
class FlareTest(unittest.TestCase):
    ...
    def test_init(self, mock_config, mock_version, mock_tempdir, mock_strftime):
        ...
        self.assertEqual(f._url, 'https://6-6-6-flare.agent.datamonitorhq.com/support/flare')
        self.assertEqual(f.tar_path, os.path.join(get_mocked_temp(), "datamonitor-agent-1.tar.bz2"))
    ...
    def test_upload_no_case(self, mock_config, mock_tempdir, mock_stfrtime, mock_version, mock_requests):
        ...
        self.assertEqual(
            args,
            ('https://6-6-6-flare.agent.datamonitorhq.com/support/flare?api_key=APIKEY',)
        )
        self.assertEqual(
            kwargs['files']['flare_file'].name,
            os.path.join(get_mocked_temp(), "datamonitor-agent-1.tar.bz2")
