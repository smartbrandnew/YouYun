...
class TestUnitmonitorStatsd(unittest.TestCase):
    ...
    def test_formatter(self):
        stats = MetricsAggregator('myhost', interval=10,
            formatter = get_formatter({"statsd_metric_namespace": "datamonitor"}))
        stats.submit_packets('gauge:16|c|#tag3,tag4')
        metrics = self.sort_metrics(stats.flush())
        self.assertTrue(len(metrics) == 1)
        self.assertTrue(metrics[0]['metric'] == "datamonitor.gauge")

        stats = MetricsAggregator('myhost', interval=10,
            formatter = get_formatter({"statsd_metric_namespace": "datamonitore."}))
        stats.submit_packets('gauge:16|c|#tag3,tag4')
        metrics = self.sort_metrics(stats.flush())
        self.assertTrue(len(metrics) == 1)
        self.assertTrue(metrics[0]['metric'] == "datamonitore.gauge")
    ...
    def test_tags_gh442(self):
        ...
        serialized = monitorstatsd.serialize_metrics([api_formatter("foo", 12, 1, ('tag',), 'host')], "test-host")
    ...
    def test_diagnostic_stats(self):
        ...
        stats.send_packet_count('datamonitor.monitorstatsd.packet.count')
        metrics = self.sort_metrics(stats.flush())
        nt.assert_equals(2, len(metrics))
        first, second = metrics

        nt.assert_equal(first['metric'], 'datamonitor.monitorstatsd.packet.count')
