...
class TestTransaction(unittest.TestCase):
    ...
    def testEndpoints(self):
        ...
        endpoints = [transaction.get_url(e) for e in transaction._endpoints]
        expected = ['https://{0}-app.agent.datamonitorhq.com/intake/?api_key={1}'.format(
        ...
        endpoints = [transaction.get_url(e) for e in transaction._endpoints]
        expected = ['https://{0}-app.agent.datamonitorhq.com/api/v1/series/?api_key={1}'.format(
            get_version().replace(".", "-"), api_key)]
        self.assertEqual(endpoints, expected, (endpoints, expected))
        ...
        endpoints = [transaction.get_url(e) for e in transaction._endpoints]
        expected = ['https://{0}-app.agent.datamonitorhq.com/api/v1/check_run/?api_key={1}'.format(
            get_version().replace(".", "-"), api_key)]
        self.assertEqual(endpoints, expected, (endpoints, expected))
