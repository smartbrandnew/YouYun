...
# project
from checks.datamonitor import monitorstreams, EventDefaults

log = logging.getLogger('datamonitor.test')
...
class TailTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.logger = logging.getLogger('test.monitorstream')
...
class Testmonitorstream(TailTestCase):
    ...
    def test_monitorstream_gauge(self):
        log_data = [
            # bucket 0
        ...
        actual_output = self.monitorstream.check(self.config, move_end=False)
        self.assertEquals(expected_output, actual_output)
        for metric, timestamp, val, attr in expected_output['monitorstream']:
            assert isinstance(val, float)

    def test_monitorstream_counter(self):
        log_data = [
            # bucket 0
    ...
    def test_monitorstream_bad_input(self):
        log_data = [
    ...
    def test_monitorstream_log_path_globbing(self):
        """Make sure that globbed monitorstream logfile matching works."""
        ...
        monitorstream_glob = os.path.join(gettempdir(), tmp_fprefix + '*')
        paths = monitorstreams._get_monitorstream_log_paths(monitorstream_glob)
        self.assertEqual(set(paths), all_tmp_filenames)

    def test_monitorstream_function_plugin(self):
        ...
        statemonitor = monitorstreams.init(self.logger, {'monitorstreams': '{0}:{1}:parse_function_plugin'.format(self.log_file.name, __name__)})
