...
class Agent(Daemon):
    ...
    def set_ip_uuid_conf(self):
        need_restart = False
        conf_file = os.path.join(get_config_path(), '..', 'conf', 'datamonitor.conf')
        ...
        with open(conf_file, 'w') as fconf:
            for line in lines:
                if '# ip:' in line:
                    ...
                    ip_string = 'ip: {}'.format(ip)
                    line = line.replace('# ip:', ip_string)
                elif '# uuid:' in line:
                    need_restart = True
                    from uyunuuid import get_uuid
                    uuid_string = 'uuid: {}'.format(get_uuid())
                    line = line.replace('# uuid:', uuid_string)
                fconf.write(line)
        return need_restart

    def set_uuid_conf(self):
        need_restart = False
        conf_file = os.path.join(get_config_path(), '..', 'conf', 'datamonitor.conf')
        with open(conf_file, 'r') as fconf:
            lines = fconf.readlines()
    ...
    def run(self, config=None):
        ...
        if self.set_ip_conf() or self.set_uuid_conf():
            self._do_restart()
