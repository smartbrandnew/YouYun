...
monitorweb_reporting_failure_url="https://app.datamonitorhq.com/agent_stats/report_failure"
email_reporting_failure="support@datamonitorhq.com"
agent_help_page="http://docs.datamonitorhq.com/guides/basic_agent_usage/"
see_agent_on_datamonitor_page="https://app.datamonitorhq.com/infrastructure"
...
A notification has been sent to Datamonitor with the content of $logfile.
...
and we'll do our very best to help you solve your problem.\n$DEFAULT"
    ...
    curl -f -s -d "version=$agent_version&os=$OS&apikey=$key_to_report&log=$encoded_log" $monitorweb_reporting_failure_url >> $logfile 2>&1 && printf "$notification_message" || report_using_mail
...
unknown_error() {
  ...
  while true; do
    read -p "Do you want to send a failure report to Datamonitor (Content of the report is in $logfile)? (y/n)" yn
    case $yn in
        [Yy]* ) report; break;;
...
if [ $dd_home ]; then
  ...
  if [ "$unamestr" = "SunOS" ]; then
      dd_base="/opt/local/datamonitor"
  else
      dd_base=$HOME/.datamonitor-agent
  fi
fi
...
logfile="$dd_base/monitoragent-install.log"
...
$dl_cmd $dd_base/requirements.txt https://raw.githubusercontent.com/Datamonitor/monitor-agent/$tag/requirements.txt  >> $logfile 2>&1
...
$dl_cmd $dd_base/requirements.txt https://raw.githubusercontent.com/Datamonitor/monitor-agent/$tag/requirements-opt.txt  >> $logfile 2>&1
...
$dl_cmd $dd_base/agent.tar.gz https://github.com/Datamonitor/monitor-agent/tarball/$tag >> $logfile 2>&1
print_done
...
printf "Configuring datamonitor.conf file......" | tee -a $logfile
if [ $apikey ]; then
    sed "s/api_key:.*/api_key: $apikey/" $dd_base/agent/datamonitor.conf.example > $dd_base/agent/datamonitor.conf 2>> $logfile
    chmod 640 $dd_base/agent/datamonitor.conf
else
...
if [ "$unamestr" = "SunOS" ]; then
    ...
    sed -i "s/# log_to_syslog: yes/log_to_syslog: no/" $dd_base/agent/datamonitor.conf 2>> $logfile
fi
printf "disable_file_logging: True" >> $dd_base/agent/datamonitor.conf

print_done

printf "Setting up launching scripts....." | tee -a $logfile
mkdir -p $dd_base/bin >> $logfile 2>&1
cp $dd_base/agent/packaging/datamonitor-agent/source/agent $dd_base/bin/agent >> $logfile 2>&1
cp $dd_base/agent/packaging/datamonitor-agent/source/info  $dd_base/bin/info >> $logfile 2>&1
...
if [ "$unamestr" = "SunOS" ]; then
    cp $dd_base/agent/packaging/datamonitor-agent/smartos/monitor-agent $dd_base/bin/monitor-agent >> $logfile 2>&1
    chmod +x $dd_base/bin/monitor-agent >> $logfile 2>&1
fi
...
cp $dd_base/agent/packaging/datamonitor-agent/source/supervisord.conf $dd_base/supervisord/supervisord.conf >> $logfile 2>&1
mkdir -p $dd_base/run
print_done
...
if [ "$unamestr" = "Darwin" ]; then
    ...
    sed "s|AGENT_BASE|$dd_base|; s|USER_NAME|$(whoami)|" $dd_base/agent/packaging/datamonitor-agent/osx/com.datamonitorhq.Agent.plist.example > $dd_base/launchd/com.datamonitorhq.Agent.plist
fi
...
if [ "$unamestr" = "SunOS" ]; then
    ...
    # SMF work now
    svccfg import $dd_base/agent/packaging/datamonitor-agent/smartos/monitor-agent.xml >> $logfile 2>&1
    svcadm enable site/datamonitor >> $logfile 2>&1
    svcs datamonitor >> $logfile 2>&1
  ...
  that data is being submitted. You should see your Agent show up in Datamonitor
  shortly at:

      $see_agent_on_datamonitor_page $DEFAULT" | tee -a $logfile
  ...
  while you're logged in, run:
      ...
      cp $dd_base/launchd/com.datamonitorhq.Agent.plist ~/Library/LaunchAgents/.
      launchctl load -w ~/Library/LaunchAgents/com.datamonitorhq.Agent.plist
  " | tee -a $logfile
      fi
