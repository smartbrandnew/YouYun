...
Please use the 1-step script available at https://app.datamonitorhq.com/account/settings#agent/mac.\033[0m\n"
    exit 1;
...
if [ $OS = "RedHat" ]; then
    echo -e "\033[34m\n* Installing YUM sources for Datamonitor\n\033[0m"
    ...
    $sudo_cmd sh -c "echo -e '[datamonitor]\nname = uyunsoft, Inc.\nbaseurl = $PROTOCOL://yum.datamonitorhq.com/rpm/$ARCHI/\nenabled=1\ngpgcheck=1\npriority=1\ngpgkey=$PROTOCOL://yum.datamonitorhq.com/DATAmonitor_RPM_KEY.public' > /etc/yum.repos.d/datamonitor.repo"

    printf "\033[34m* Installing the Monitor Agent package\n\033[0m\n"

    if $DDBASE; then
        DD_BASE_INSTALLED=$(yum list installed datamonitor-agent-base > /dev/null 2>&1 || echo "no")
        if [ "$DD_BASE_INSTALLED" != "no" ]; then
            echo -e "\033[34m\n* Uninstall datamonitor-agent-base\n\033[0m"
            $sudo_cmd yum -y remove datamonitor-agent-base
        fi
    fi
    $sudo_cmd yum -y --disablerepo='*' --enablerepo='datamonitor' install datamonitor-agent
elif [ $OS = "Debian" ]; then
    printf "\033[34m\n* Installing apt-transport-https\n\033[0m\n"
    $sudo_cmd apt-get update
    $sudo_cmd apt-get install -y apt-transport-https
    printf "\033[34m\n* Installing APT package sources for Datamonitor\n\033[0m\n"
    $sudo_cmd sh -c "echo 'deb http://apt.datamonitorhq.com/ stable main' > /etc/apt/sources.list.d/datamonitor.list"
    $sudo_cmd apt-key adv --recv-keys --keyserver hkp://keyserver.ubuntu.com:80 C7A7DA52

    printf "\033[34m\n* Installing the Monitor Agent package\n\033[0m\n"
    ERROR_MESSAGE="ERROR
Failed to update the sources after adding the Datamonitor repository.
This may be due to any of the configured APT sources failing -
see the logs above to determine the cause.
If the failing repository is Datamonitor, please contact Datamonitor support.
*****
"
    $sudo_cmd apt-get update -o Dir::Etc::sourcelist="sources.list.d/datamonitor.list" -o Dir::Etc::sourceparts="-" -o APT::Get::List-Cleanup="0"
    ERROR_MESSAGE="ERROR
Failed to install the Datamonitor package, sometimes it may be
due to another APT source failing. See the logs above to
determine the cause.
If the cause is unclear, please contact Datamonitor support.
*****
"
    $sudo_cmd apt-get install -y --force-yes datamonitor-agent
    ERROR_MESSAGE=""
else
    printf "\033[31mYour OS or distribution are not supported by this install script.
Please follow the instructions on the Agent setup page:

    https://app.datamonitorhq.com/account/settings#agent\033[0m\n"
    exit;
fi

# Set the configuration
if [ -e /etc/monitor-agent/datamonitor.conf ]; then
    ...
    if [ $dd_hostname ]; then
        ...
        $sudo_cmd sh -c "sed -i 's/#hostname:.*/hostname: $dd_hostname/' /etc/monitor-agent/datamonitor.conf"
    fi
    $sudo_cmd chown monitor-agent:root /etc/monitor-agent/datamonitor.conf
