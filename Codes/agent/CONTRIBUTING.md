...
- You can first take a look at the [Troubleshooting](https://datamonitor.zendesk.com/hc/en-us/sections/200766955-Troubleshooting) section of our [Knowledge base](https://datamonitor.zendesk.com/hc/en-us).
- If you can't find anything useful, please contact our [support](http://docs.datamonitorhq.com/help/) and [send them your logs](https://github.com/Datamonitor/monitor-agent/wiki/Send-logs-to-support).
...
Your Pull Request **must** always pass the Travis/Appveyor tests before being merged, if you think the error is not due to your changes, you can have a talk with us on IRC (#datamonitor freenode) or send us an email to support _at_ datamonitorhq _dot_ com)
...
Avoid changing too many things at once, for instance if you're fixing the redis integration and at the same time shipping a monitorstatsd improvement, it makes reviewing harder (devs specialize in different parts of the code) and the change _time-to-release_ longer.
...
Examples, see:
  * https://github.com/Datamonitor/monitor-agent/commit/44bc927aaaf2925ef081768b5888bbb20a5bb3bd
  * https://github.com/Datamonitor/monitor-agent/commit/677417fe12b1914e4322ac2c1fd1645cb0f1de31
...
* _monitorstatsd_: for the embedded monitorstatsd server
...
We define what are the agent dependencies in the [monitor-agent-omnibus](https://github.com/Datamonitor/monitor-agent-omnibus) repository, and we define how to build/add these dependencies in the [omnibus-software](https://github.com/Datamonitor/omnibus-software) repository.
...
#### monitor-agent-omnibus

First, fork `omnibus-software`, and create your branch.

Then add `dependency "my_module"` to `config/projects/datamonitor-agent.rb`: (please respect alphabetical sort)
```ruby
# Check dependencies
...
If you need to install it with `python setup.py install`, take a look at this [example](https://github.com/Datamonitor/omnibus-software/blob/macos-clean/config/software/guidata.rb). (which also demonstrate the use of a patch).
