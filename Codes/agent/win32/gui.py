...
MAIN_WINDOW_TITLE = "Monitor Agent Manager"

DATAmonitor_SERVICE = "MonitorAgent"
...
class DatamonitorConf(EditorFile):
    def __init__(self, config_path):
        EditorFile.__init__(self, config_path, "Agent settings file: datamonitor.conf")
    ...
    def check_api_key(self, editor):
        if self.api_key is None:
            api_key, ok = QInputDialog.getText(
                ...
                " You can find it here: https://monitor.uyun.cn/#/deploy"
            )
            if ok and api_key:
                new_content = []
                ...
                warning_popup("The agent needs an API key to send metrics to Datamonitor")
                if agent_status() != AGENT_STOPPED:
                    agent_manager("stop")
            else:
...
class PropertiesWidget(QWidget):
    ...
    def set_datamonitor_conf(self, datamonitor_conf):
        self.save_button.setEnabled(True)
        self.refresh_button.setEnabled(False)
        self.current_file = datamonitor_conf
        self.desc_label.setText(datamonitor_conf.get_description())
        self.editor.set_text_from_file(datamonitor_conf.file_path)
        datamonitor_conf.content = self.editor.toPlainText().__str__()
        self.disable_button.setEnabled(False)
        self.enable_button.setEnabled(False)
        datamonitor_conf.check_api_key(self.editor)
...
class HTMLWindow(QTextEdit):
    ...
    def latest_status(self):
        try:
            ...
            monitorstatsd_status = MonitorstatsdStatus.load_latest_status()
            ...
            generated_template = loaded_template.load("status.html").generate(
                ...
                monitorstatsd=monitorstatsd_status.to_dict(),
...
class MainWindow(QSplitter):
    def __init__(self, parent=None):
        ...
        checks = get_checks()
        datamonitor_conf = DatamonitorConf(get_config_path())
        ...
        self.settings = [
            ...
            ("Collector Logs", lambda: [self.properties.set_log_file(self.collector_log_file),
                self.show_html(self.properties.group_code, self.properties.html_window, False)]),
            ("Monitorstatsd Logs", lambda: [self.properties.set_log_file(self.monitorstatsd_log_file),
                self.show_html(self.properties.group_code, self.properties.html_window, False)]),
        ...
        self.connect(self.agent_settings, SIGNAL("clicked()"),
                     lambda: [self.properties.set_datamonitor_conf(datamonitor_conf),
                     self.show_html(self.properties.group_code, self.properties.html_window, False)])
        ...
        self.properties.set_datamonitor_conf(datamonitor_conf)
    ...
    def create_logs_files_windows(self, config, prefix):
        ...
        self.monitorstatsd_log_file = EditorFile(
            config.get('%smonitorstatsd_log_file' % prefix),
            "monitorstatsd log file"
        )
...
class Menu(QMenu):
    ...
    def enable_or_disable_mac(self):
        try:
            output = check_output(['osascript', '-e',
                                   self.SYSTEM_EVENTS_CMD.format('get the path of every login item whose name is "Monitor Agent"')])
            return 'Disable' if 'Datamonitor' in output else 'Enable'
    ...
    def enable_or_disable_login(self):
        ...
        if previous == 'Disable':
            command = 'delete every login item whose name is "Monitor Agent"'
        else:
            command = 'make login item at end with properties {path:"/Applications/Monitor Agent.app", name:"Monitor Agent", hidden:false}'
        try:
...
def service_manager(action):
    try:
        if action == 'stop':
            win32serviceutil.StopService(DATAmonitor_SERVICE)
        elif action == 'start':
            win32serviceutil.StartService(DATAmonitor_SERVICE)
        elif action == 'restart':
            win32serviceutil.RestartService(DATAmonitor_SERVICE)
    except Exception, e:
...
def service_manager_status():
    try:
        return WIN_STATUS_TO_AGENT[
            win32serviceutil.QueryServiceStatus(DATAmonitor_SERVICE)[1]
        ]
    except Exception:
...
def osx_manager(action):
    try:
        check_call(['datamonitor-agent', action])
    except Exception, e:
        warning_popup("Couldn't execute datamonitor-agent %s: \n %s" % (action, str(e)))


def osx_manager_status():
    try:
        check_output(['datamonitor-agent', 'status'])
...
def windows_flare():
    case_id, ok = QInputDialog.getInteger(
        ...
        "sent to Datamonitor Support. Please enter your ticket number if you have one:",
        value=0, min=0
    )
    if not ok:
...
if __name__ == '__main__':
    ...
    if Platform.is_mac():
        ...
        # add datamonitor-agent in PATH
        os.environ['PATH'] = "{0}:{1}".format(
            '/opt/datamonitor-agent/bin/',
            os.environ['PATH']
        )
