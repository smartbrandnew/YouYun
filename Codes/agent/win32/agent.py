...
class AgentSvc(win32serviceutil.ServiceFramework):
    ...
    _svc_description_ = "Sends metrics to Datamonitor"
    ...
    def __init__(self, args):
        ...
        try:
            ...
            log.info("Not starting net scan(net collector): no valid configuration found")
        else:
            net_collector = NetCollectorProcess(agentConfig, self.hostname, net_config=config)
            self.procs.update({"net_collector": ProcessWatchmonitor("net_collector", net_collector)})
...
class NetCollectorProcess(multiprocessing.Process):
    def __init__(self, agentConfig, hostname, **options):
        multiprocessing.Process.__init__(self, name='net_collector')
        self.config = agentConfig
        self.hostname = hostname
        self.is_enabled = True
        self.options = options
        self.net_config = self.options['net_config']['instances'][0]
        self.running = True

    def run(self):
        from config import initialize_logging
        initialize_logging('windows_net_collector')
        log.debug("Windows Service - Starting net_collector")
        emitters = self.get_emitters()
    ...
    def stop(self):
        log.debug("Windows Service - Stopping net_collector")
        self.collector.stop()
        self.running = False
        log.debug("Windows Service - Stopped net_collector - Exit by command")
...
class UpdaterProcess(multiprocessing.Process):
    def __init__(self, config, hostname, **options):
        ...
        self.linux_conf = '/opt/datadog-agent/'
        ...
        self.dirs = {'conf': 'C:/ProgramData/Datamonitor/',
                     'checks': 'C:/Program Files (x86)/Datamonitor/Monitor Agent/'
        }
        if os.path.exists('C:/ProgramData/Datamonitor/conf.d/') == False:
            self.windows_conf = 'C:/Documents and Settings/All Users/Application Data/Datamonitor/'
            self.dirs['conf'] = 'C:/Documents and Settings/All Users/Application Data/Datamonitor/'
        if os.path.exists('C:/Program Files (x86)/Datamonitor/Monitor Agent/'):
            self.dirs['checks'] = 'C:/Program Files (x86)/Datamonitor/Monitor Agent/'
        elif os.path.exists('C:/Program Files/Datamonitor/Monitor Agent/'):
            self.dirs['checks'] = 'C:/Program Files/Datamonitor/Monitor Agent/'
    ...
    def updater(self):
        while self.running:
            ...
            try:
                ...
                au.do_update()
                os.chdir(self.dirs["checks"] + "files/")
                os.system("monitoragent.exe restart")
    ...
    def configurator(self):
        while self.running:
            ...
            try:
                cc = CentralConfigurator("Windows", self.linux_conf, self.windows_conf, self.central_configuration_url, self.central_configuration_api_key)
