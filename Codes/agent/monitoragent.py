...
WATCHmonitor_INTERVAL_MULTIPLIER = 10  # 10x flush interval
HEADERS_TO_REMOVE = [
    'Host',
    'Content-Length',
]
...
class AgentTransaction(Transaction):
    ...
    def set_endpoints(cls):
        """
        Set Datamonitor endpoint if an API key exists.
        """
    ...
    def flush(self):
        for endpoint in self._endpoints:
            ...
            if use_curl:
                if pycurl is None:
                    log.error("monitor-agent is configured to use the Curl HTTP Client, but pycurl is not available on this system.")
                else:
...
class Application(tornado.web.Application):
    ...
    def _postAgentInfoToServer(self):
        ...
        url = self._agentConfig['m_url'].replace('api/v2/gateway/dd-agent','api/v2/agent/info/intake?api_key=') + self._agentConfig['api_key']
        modified = time.mktime(time.localtime(time.time()))

        path = ''
        if platform.system() == 'Linux':
            path = _unix_confd_path()
        elif platform.system() == 'Windows' and os.path.exists('C:/ProgramData/Datamonitor/conf.d'):
            path = _windows_confd_path()
        elif platform.system() == 'Windows' and os.path.exists('C:/Documents and Settings/All Users/Application Data/Datamonitor'):
