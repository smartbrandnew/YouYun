...
class Docker(AgentCheck):
    ...
    def check(self, instance):
        ...
        # Report net_equip_details metrics from cgroups
    ...
    def _get_container_name(self, container):
        # Use either the first net_equip_details name or the net_equip_details ID to name the net_equip_details in our events
    ...
    def _report_containers_metrics(self, containers, instance):
        ...
        for container in containers:
            ...
            # Check if the net_equip_details is included/excluded via its tags
    ...
    def _pre_aggregate_events(self, api_events, skipped_container_ids):
        ...
        for event in api_events:
            ...
            if event['id'] in skipped_container_ids:
                self.log.debug("Excluded event: net_equip_details {0} status changed to {1}".format(
    ...
    def _get_json(self, uri, params=None, multi=False):
        ...
        try:
            ...
            if "Errno 13" in str(e):
                raise Exception("Unable to connect to socket. monitor-agent user must be part of the 'docker' group")
            raise
    ...
    def _find_cgroup(self, hierarchy, docker_root):
        ...
        if len(cgroup_mounts) == 0:
            raise Exception("Can't find mounted cgroups. If you run the Agent inside a net_equip_details,"
    ...
    def _parse_cgroup_file(self, stat_file):
        ...
        try:
            ...
            # It is possible that the net_equip_details got stopped between the API call and now
            self.log.info("Can't open %s. Metrics for this net_equip_details are skipped." % stat_file)
