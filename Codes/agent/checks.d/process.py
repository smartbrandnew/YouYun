...
class ProcessCheck(AgentCheck):
        ...
        for proc in psutil.process_iter():
            ...
            for string in search_string:
                try:
                    ...
                    if found:
                        self.gauge('system.processes.status', 1, tags=['running:%s' % name, name])
                        matching_pids.add(proc.pid)
                        break
        
        if not found:
            self.gauge('system.processes.status', 0, tags=['quit:%s' % name, name])
    ...
    def get_process_state(self, name, pids, collect_events):
        st = defaultdict(list)
        status = {}
        ...
        for pid in pids:
            ...
            if collect_events:
                status_info = self.psutil_wrapper(p, 'status', None)
                if status_info and self.status_temp.get(pid) != status_info:
                    status[pid] = status_info
                    self.status_temp[pid] = status_info
        # Get the "quit" status for the missed process pids.
        pid_key_diff = set(self.status_temp.keys()) - pids
        for pid_key in pid_key_diff:
            status[pid_key] = "quit"
            del self.status_temp[pid_key]

        return st, status

    def _format_events(self, ev, name, tags):
        events = []
        for pid, status in ev.items():
            timestamp = time.time()
            msg_title = "Process name %s pid %s now %s" % (name, str(pid), status)
            msg_body = "Process pid %s now %s" % (str(pid), status)
            events.append({
                'timestamp': timestamp,
                'host': self.hostname,
                ...
                'msg_title': msg_title,
                'msg_text': msg_body,
                "alert_type": STATUS_ALERT[status],
                "tags": tags
            })
    ...
    def check(self, instance):
        ...
        collect_events = instance.get('collect_events', False)
        ...
        proc_state, proc_status = self.get_process_state(name, pids, collect_events)
        ...
        if proc_status:
            events = self._format_events(proc_status, name, tags)
            for ev in events:
