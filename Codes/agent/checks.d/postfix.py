...
class PostfixCheck(AgentCheck):
    ...
    WARNING: the user that monitor-agent runs as must have sudo access for the 'find' command
             sudo access is not required when running monitor-agent as root (not recommended)
    ...
    def _get_queue_count(self, directory, queues, tags):
        for queue in queues:
            ...
            count = 0
            if os.geteuid() == 0:
                # monitor-agent is running as root (not recommended)
                count = sum(len(files) for root, dirs, files in os.walk(queue_path))
            else:
                # can monitor-agent user run sudo?
