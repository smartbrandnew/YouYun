...
UNIX_CONFIG_PATH = '/etc/monitor-agent'
...
def _windows_config_path():
    ...
    return _config_path(os.path.join(common_data, 'Datamonitor'))


def _windows_confd_path():
    common_data = _windows_commondata_path()
    return _confd_path(os.path.join(common_data, 'Datamonitor'))
...
def get_logging_config(cfg_path=None):
    ...
    if system_os == 'windows':
        logging_config['windows_collector_log_file'] = os.path.join(_windows_commondata_path(), 'Datamonitor', 'logs', 'collector.log')
        logging_config['windows_net_collector_log_file'] = os.path.join(_windows_commondata_path(), 'Datamonitor', 'logs', 'net_collector.log')
        logging_config['windows_script_caller_log_file'] = os.path.join(_windows_commondata_path(), 'Datamonitor', 'logs', 'script_caller.log')
        logging_config['windows_updater_log_file'] = os.path.join(_windows_commondata_path(), 'Datamonitor', 'logs', 'updater.log')
        logging_config['windows_forwarder_log_file'] = os.path.join(_windows_commondata_path(), 'Datamonitor', 'logs', 'forwarder.log')
        logging_config['windows_monitorstatsd_log_file'] = os.path.join(_windows_commondata_path(), 'Datamonitor', 'logs', 'monitorstatsd.log')
        logging_config['jmxfetch_log_file'] = os.path.join(_windows_commondata_path(), 'Datamonitor', 'logs', 'jmxfetch.log')
    ...
    #     if system_os == 'windows':
    #         config_example_file = "https://github.com/Datamonitor/monitor-agent/blob/master/packaging/datamonitor-agent/win32/install_files/datamonitor_win32.conf"
    #     else:
    #         config_example_file = "https://github.com/Datamonitor/monitor-agent/blob/master/datamonitor.conf.example"
    #
    #     sys.stderr.write("""Python logging config is no longer supported and will be ignored.
    #         To configure logging, update the logging portion of 'datamonitor.conf' to match:
    #          '%s'.
    #          """ % config_example_file)
