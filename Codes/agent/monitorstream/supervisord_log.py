"""
Custom parser for supervisord log suitable for use by Datamonitor 'monitorstreams'

Add to datamonitor.conf as follows:

monitorstreams: [path_to_supervisord.log]:datamonitor.streams.supervisord:parse_supervisord
...
# mapping between datamonitor and supervisord log levels
ALERT_TYPES_MAPPING = {
    "CRIT": "error",
    "ERRO": "error",
    "WARN": "warning",
...
def parse_supervisord(log, line):
    """
    Parse the supervisord.log line into a monitorstream event
    """
    if len(line) == 0:
