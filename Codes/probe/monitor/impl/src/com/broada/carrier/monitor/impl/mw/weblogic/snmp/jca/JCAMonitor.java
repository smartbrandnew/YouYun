...
public class JCAMonitor extends BaseMonitor {
	...
	public Serializable collect(CollectContext context) {
		MonitorResult result = new MonitorResult(
				MonitorConstant.MONITORSTATE_NICER);
		JCA jca = new JCA();

		long replyTime = System.currentTimeMillis();

		jca.setAbilityScale(new BigDecimal(100 - Math.random() * MAX_RATE)
				.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
		jca.setBlabScale(new BigDecimal(Math.random() * MAX_RATE).setScale(2,
		...
		result.addPerfResult(new PerfResult(ITEM_ABILITY_SCALE, jca
				.getAbilityScale()));
