...
public class WASParamPanel extends BaseMethodConfiger {
	...
	public static String USER_DIER = System.getProperty("user.dir").replaceAll(
			"\\\\", "/")
			+ "/";
	public static String CERPATH = System.getProperty("user.dir").replaceAll(
			"\\\\", "/")
			+ "/conf/cer/";
	...
	private SpinnerNumberModel was_port = new SpinnerNumberModel(9080, 1,
			65535, 1);
	private SpinnerNumberModel connector_port = new SpinnerNumberModel(8880, 1,
			65535, 1);
	...
	public void setOptions(WASMonitorMethodOption option) {
		...
		if (chkssl) {
			if (option.getServerCerFilePath() == null
					|| option.getServerCerFilePath() == CERPATH
					|| option.getServerCerFilePath().equals(CERPATH)) {
				jTextSSLServerCer.setText("");
			} else {
				jTextSSLServerCer.setText(USER_DIER
						+ option.getServerCerFilePath());
			}
			if (option.getClientKeyPath() == null
					|| option.getClientKeyPath() == CERPATH
					|| option.getClientKeyPath().equals(CERPATH)) {
				jTextSSLClientKey.setText("");
			} else {
				jTextSSLClientKey
						.setText(USER_DIER + option.getClientKeyPath());
			}
	...
	public WASMonitorMethodOption getOptions() {
		...
		option.setConnectorType(jComboxConnectorType.getSelectedItem()
				.toString());
		option.setConnectorHost(jTextConnectorHost.getText().trim());

		// ssl认证配置
		String serverCerPath = jTextSSLServerCer.getText().trim()
				.replaceAll("\\\\", "/");
		String clientKeyPath = jTextSSLClientKey.getText().trim()
				.replaceAll("\\\\", "/");
		...
		if (jChkSSL.isSelected()) {
			try {
				...
				// ErrorDlg.createErrorDlg(this, "同步证书到探针出错，证书文件必须放在目录：" +
				// CERPATH + "或者其子目录中。", e).setVisible(true);
	...
	public boolean verify() {
		if (new String(was_port.getNumber().toString()).trim().length() == 0) {
			int opt = JOptionPane.showConfirmDialog(this, "端口真的为空吗？", "提问？",
	...
	private void doTest() {
		...
		try {
			...
			if (method == null)
				return;
			Object ret = getContext()
					.getServerFactory()
					.getProbeService()
					.executeMethod(getContext().getNode().getProbeId(),
							WASTest4Probe.class.getName(), "test",
							getContext().getNode().getIp(), method);
			if (null == ret) {
				JOptionPane.showMessageDialog(this, "成功连接到Websphere服务器", "成功",
	...
	private String[] synCert(String _serverCer, String _clinetKey)
			throws Exception {
		...
		String scerPath = scer.getPath();
		if(!scerPath.startsWith(WorkPathUtil.getRootPath())){
			scerPath = WorkPathUtil.getRootPath() + scerPath;
		}
		String ccerPath = ccer.getPath();
		if(!ccerPath.startsWith(WorkPathUtil.getRootPath())){
			ccerPath = WorkPathUtil.getRootPath() + ccerPath;
		}
		if (!StringUtil.isNullOrBlank(_serverCer)
				&& (!scer.exists() || !scer.isFile() || !scer.canRead())) {
			throw new RuntimeException("证书文件[" + scerPath + "不存在或者无读取权限。");
		}
		if (scer.exists() && !isFileInDirectory(scer, cerDir)) {
			/*
			 * JOptionPane.showMessageDialog(this, "证书文件必须放在目录：" + cerDir +
			 * "或者其子目录中。", "信息", JOptionPane.INFORMATION_MESSAGE); return null;
			 */
			throw new RuntimeException("同步证书到探针出错，证书文件必须放在目录：" + CERPATH
					+ "或者其子目录中。");
		}

		if (!StringUtil.isNullOrBlank(_clinetKey)
				&& (!ccer.exists() || !ccer.isFile() || !ccer.canRead())) {
			throw new RuntimeException("证书文件[" + ccerPath + "不存在或者无读取权限。");
		}
		...
		try {
			...
			// 同步证书到probe
			int idx = new File(System.getProperty("user.dir", ".")).getPath()
					.replaceAll("\\\\", "/").length() + 1;
			if (scer.exists() && scer.isFile()) {
				sccer[0] = scerPath.replaceAll("\\\\", "/").substring(idx);
				// server导入服务器证书到trustedstore,在同步配置文件的时候，该trutstore中将包含该证书
				WASUtil.importServerCer2Trusted(sccer[0]);
				getContext()
						.getServerFactory()
						.getProbeService()
						.uploadFile(getContext().getNode().getProbeId(),
								sccer[0], "conf/cer/");
				getContext()
						.getServerFactory()
						.getProbeService()
						.executeMethod(getContext().getNode().getProbeId(),
								WASUtil.class.getName(),
								"importServerCer2Trusted", sccer[0]);
			}
			if (ccer.exists() && ccer.isFile()) {
				sccer[1] = ccer.getCanonicalPath().replaceAll("\\\\", "/")
						.substring(idx);
				getContext()
						.getServerFactory()
						.getProbeService()
	...
	private void chooseServerCer(ActionEvent e) {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			String serverCerPath = jfcSSLServerCer.getSelectedFile().getPath();
			if(!serverCerPath.startsWith(WorkPathUtil.getRootPath())){
				serverCerPath = WorkPathUtil.getRootPath() + serverCerPath;
			}
			jTextSSLServerCer.setText(serverCerPath);
			// String p = "conf/cer/" +
			// jfcSSLServerCer.getSelectedFile().getName();
	...
	private void chooseClientKey(ActionEvent e) {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			String clientKeyPath = jfcSSLClientKey.getSelectedFile().getPath();
			if(!clientKeyPath.startsWith(WorkPathUtil.getRootPath())){
				clientKeyPath = WorkPathUtil.getRootPath() + clientKeyPath;
			}
			jTextSSLClientKey.setText(clientKeyPath);
			// String p = "conf/cer/" +
			// jfcSSLClientKey.getSelectedFile().getName();
	...
	private static boolean isFileInDirectory(File f1, File d)
			throws IOException {
		boolean ret = false;
