...
public class OracleRACMethodConfiger extends BaseMethodConfiger {
	...
	public OracleRACMethodConfiger() {
		this.setLayout(null);
		...
		jButtonTest.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String value;
				try {
					OracleRACMethod method = getOracleRACMethod();
					if (method == null)
						return;					
					value = (String) getContext().getServerFactory().getProbeService()
							.executeMethod(getContext().getNode().getProbeId(), OracleRACTester.class.getName(), "doTest",
				...
				if (Boolean.parseBoolean(value)) {
					JOptionPane.showMessageDialog(OracleRACMethodConfiger.this, "连接数据库测试成功");
				} else {
					JOptionPane.showMessageDialog(OracleRACMethodConfiger.this, "连接数据库测试失败：" + value);
				}
			}
		});
	}

	private OracleRACMethod getOracleRACMethod() {
		OracleRACMethod option = new OracleRACMethod();
	...
	public void setData(MonitorMethod method) {
		OracleRACMethod option = new OracleRACMethod(method);
