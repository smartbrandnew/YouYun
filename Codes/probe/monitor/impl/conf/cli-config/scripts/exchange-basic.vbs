...
Set colItems = wbemServices.ExecQuery("select ActiveConnectionCount, ActiveUserCount, ClientRPCsFailed, ClientRPCsSucceeded, ExchmemCurrentBytesAllocated from Win32_PerfFormattedData_MSExchangeIS_MSExchangeIS")
...
For Each row in colItems
	WScript.Echo row.ActiveConnectionCount _
		& "|" & row.ActiveUserCount _
		& "|" & row.ClientRPCsFailed _
		& "|" & row.ClientRPCsSucceeded _
		& "|" & row.ExchmemCurrentBytesAllocated
Next
