...
class AgentStatus(object):
    ...
    def _get_pickle_path(cls):
        if Platform.is_win32():
            path = os.path.join(_windows_commondata_path(), 'run')
...
class CollectorStatus(AgentStatus):
    ...
    def body_lines(self):
        ...
        try:
            ...
            lines.append(
                '  ' + style('NTP offset', *ntp_styles) + ': ' + style('%s s' % round(ntp_offset, 4), *ntp_styles))
        except Exception, e:
    ...
    def to_dict(self):
        ...
        try:
            ...
            uuid = "Unknown (%s)" % str(e)
        status_info["uuid"] = uuid

        return status_info


class MonitorstatsdStatus(AgentStatus):
    NAME = 'monitorstatsd'
