/*!Action
 ...
 monitor.output=EMCCELERRA-CIFSSERVER-INFO
...
def executeMonitor() {
	...
	def cifsServerInstances = EMCCelerraUtils.findCelerraCIFSServer($smis);
	cifsServerInstances.each{cifsServerInst->
	def cifsServerName = $smis.getProperty(cifsServerInst,'Name');
	def result = $result.create(cifsServerName);
	result.clazz = 'CIFSServer';
	result.rs.ComponentOf = 'node';
	
	result.state.available_status = fitchDedicated($smis.getProperty(cifsServerInst,'OperationalStatus'));
	result.attr.serverDedicated = $smis.getProperty(cifsServerInst,'Dedicated');
