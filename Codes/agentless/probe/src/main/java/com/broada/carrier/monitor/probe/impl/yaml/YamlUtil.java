	...
	private static void generateMonitorNode(YamlBean yamlBean, String monitorType, String filename) {
		...
		for (YamlHost host : hosts) {
			...
			if ("127.0.0.1".equals(ip))
				throw new RuntimeException("文件：" + filename + "里的hosts标签中的ip不能配置成127.0.0.1");
		...
		for (YamlHost host : hosts) {
			...
			if (method != null) {
				if (("ProtocolCcli".equalsIgnoreCase(method.getTypeId()) || ("ProtocolDb2".equalsIgnoreCase(method
						.getTypeId()) && StringUtils.isNotNullAndTrimBlank((String) method.getProperties().get(
						"sysname"))))
						&& !"dynamic_monitor".equalsIgnoreCase(monitorType)) {
				...
				for (String typeId : typeIds) {
					...
					if (host.getDynamic_properties() != null) {
						...
						if (!groovyFile.exists())
							throw new RuntimeException(String.format("监测脚本文件%s丢失", groovyFile.getAbsolutePath()));
