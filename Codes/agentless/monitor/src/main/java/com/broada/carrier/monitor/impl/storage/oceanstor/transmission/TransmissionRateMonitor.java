package com.broada.carrier.monitor.impl.storage.oceanstor.transmission;
...
public class TransmissionRateMonitor extends BaseMonitor {
	...
	public static final String RATE = "6";
	public static final String RATE_UTILIZATION = "OCEANSTOR9000-RATE-";
	...
	public Serializable collect(CollectContext context) {
		...
		if(StringUtil.isNullOrBlank(cluster_id)){
			result.setResultDesc("通过SNMP方式未能查询到传输速率信息,缺少cluster_id的配置");
			return result;
		}
		...
		if(arrays != null && arrays.length > 0)
			for(String clusterId:arrays){
				try {
					Map<String, String> map = getTransferRateUtilization(walk, clusterId, clusterId.length());
					if(map.isEmpty()){
						result.setResultDesc("通过SNMP方式未能查询到传输速率信息");
						return result;
					}
					...
					LOG.error("通过SNMP方式查询传输速率异常，" + e.getMessage());
					result.setResultDesc("通过SNMP方式查询传输速率信息异常");
				}
			}
		return result;
	}
	 ...
	 */
	private Map<String, String> getTransferRateUtilization(SnmpGet walk, String clusterId, int length) throws SnmpException{
		Map<String, String> ret = new HashMap<String, String>();
		SnmpResult result = walk.snmpSynGet(new SnmpOID(ToAscii.getOid(PREFIX_TABLE_ID, RATE, String.valueOf(length), ToAscii.String2Ascii(clusterId))));
		if(result != null){
			ret.put("transferrate", result.getValue().toString());
		}
		return ret;
	}
