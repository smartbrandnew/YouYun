 ...
 */
public class WSNDPmiMonitor implements Monitor {
	private static final Logger logger = LoggerFactory.getLogger(WSNDPmiMonitor.class);
	...
	private MonitorResult collect(CollectContext context, MonitorTempData lastData) {
		...
		if (version == null) {
			...
			if (version == null) {
				try {	      	
					version = WebSphereGroupFacade.getWASVersion(host, port, username, password, params, true);
		...
		WebSphere webSphere;
		try {
			webSphere = WebSphereGroupFacade.getWebSphereByVersion(version, true);
		} catch (Exception e) {
		...
		Map<String, Item> items = webSphere.getType(typeId).getItems();
		...
		try {
			...
			WebSphereGroupFacade.link(WebSphereGroupFacade.getDefaultLinkUrl(true), params);
		...
		try {
			Map<String, WASMonitorResult> perfResults = WebSphereGroupFacade.getAllWASMonitorResults(version, typeId, host,
					port, null, null, username, password, params, true);

			if (perfResults == null || perfResults.isEmpty()) {
				logger.error("获取Websphere性能数据失败，结果为空");
			...
			for (Entry<String, WASMonitorResult> entry : perfResults.entrySet()) {
				...
				List<String> perfIndexList = mr.toPerfIndexList();//[WASWEBAPP-4, WASWEBAPP-6, WASWEBAPP-2, WASWEBAPP-5, WASWEBAPP-1, WASWEBAPP-3]
