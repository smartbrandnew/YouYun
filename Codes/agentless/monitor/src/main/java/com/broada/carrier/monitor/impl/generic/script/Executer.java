...
public class Executer {
	...
	private void addContextWithMethod(Bindings bindings, MonitorMethod method,
		...
		if (method.getTypeId().equals(SnmpMethod.TYPE_ID)) {
			...
			option.setPassword(properties.get("password", ""));
	...
	private static void usage() {
		...
		System.out.println("\tscriptexec [options] scriptfile");
		System.out.println("Options: ");
		System.out.println("\t-s x.x.x.x\t指定结果回转服务IP");
		System.out.println("\t-p n\t\t指定结果回转服务端口");
		System.out.println("\t-i xxxx\t\t指定执行id");
		System.exit(1);
	}
}
