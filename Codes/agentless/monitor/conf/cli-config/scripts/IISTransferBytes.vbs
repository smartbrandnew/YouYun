...
On Error Resume Next
Set colItems = wbemServices.ExecQuery( "Select ServiceUptime,BytesTotalPersec,BytesReceivedPersec,BytesSentPersec,Name from Win32_PerfRawData_W3SVC_WebService")
...
If -2147217392=Err.Number Then '无效类别
	WScript.Echo vbCrLf & "Error # 获取IIS传输字节信息时发生错误，系统不支持此WMI服务，请确定IIS是否被正确安装。"
	WScript.Quit(0)
...
if Err.Number Then
	WScript.Echo vbCrLf & "Error # 获取IIS传输字节信息时发生错误:" &" " & Err.Description
