...
Set colItems = wbemServices.ExecQuery("select name, MessagesDelivered, MessagesSent, MessagesSubmitted, ReceiveQueueSize from Win32_PerfFormattedData_MSExchangeIS_MSExchangeISMailbox")
...
For Each row in colItems
	WScript.Echo row.name _
		& "|" & row.MessagesDelivered _
		& "|" & row.MessagesSent _
		& "|" & row.MessagesSubmitted _
		& "|" & row.ReceiveQueueSize
Next
