...
Set colItems = wbemServices.ExecQuery("select Name, BytesSentPersec, BytesSentTotal, MessageBytesSentPersec, MessageBytesSentTotal, MessagesSentPersec, MessagesSentTotal  from  Win32_PerfFormattedData_MSExchangeTransportSmtpSend_MSExchangeTransportSmtpSend")
...
For Each row in colItems
	WScript.Echo row.Name _
		& "|" & row.BytesSentPersec _
		& "|" & row.BytesSentTotal _
		& "|" & row.MessageBytesSentPersec _
		& "|" & row.MessageBytesSentTotal _
		& "|" & row.MessagesSentPersec _
		& "|" & row.MessagesSentTotal
Next
