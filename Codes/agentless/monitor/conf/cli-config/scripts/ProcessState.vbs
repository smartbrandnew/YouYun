...
'get system tiem
On Error Resume Next
Set colItems = wbemServices.ExecQuery( "Select * from Win32_LocalTime")
...
systimeStr = objItem.Year &"-"& objItem.Month &"-"& objItem.Day &" "& objItem.Hour &":"& objItem.Minute &":"& objItem.Second
Next
...
For Each objItem in colItems
	if objItem.CreationDate <> "" then
		Wscript.Echo objItem.Name & "|" & objItem.PageFileUsage & "|" & objItem.CreationDate & "|" & systimeStr
	else
		Wscript.Echo objItem.Name & "|" & objItem.PageFileUsage & "|" & "empty" & "|" & systimeStr
	end if
Next
'end
